...
class Graph(object):
    ...
    def get_vertex_by_id(self, vertex_id):
        ...
        if vertex_id not in self._vertices:
            raise GraphNotContainsVertexException(graph_not_contains_vertex_message(vertex_id))
    ...
    def add_edge(self, from_vertex, to_vertex, weight=0):
        ...
        if from_vertex not in self._vertices:
            raise GraphNotContainsVertexException(graph_not_contains_vertex_message(from_vertex))
        if to_vertex not in self._vertices:
            raise GraphNotContainsVertexException(graph_not_contains_vertex_message(to_vertex))
